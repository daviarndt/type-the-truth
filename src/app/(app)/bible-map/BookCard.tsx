...
export function BookCard({
  ...
  return (
    <div
      className={`bible-map-book${percent === 100 ? " bible-map-book--done" : ""}${expanded ? " bible-map-book--expanded" : ""}`}
      ...
      <div onClick={() => setExpanded((v) => !v)}>
        <div className="book-card-top">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            {percent > 0 && (
              <span className="book-card-percent" style={{ fontVariantNumeric: "tabular-nums" }}>{percent}%</span>
            )}
        ...
        <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 2 }}>
          <span className="book-card-chapters">{completed}/{totalChapters} cap.</span>
          {versesTyped > 0 && (
            <span style={{ fontSize: ".6rem", color: "hsl(var(--gold))", opacity: 0.85 }}>
              {versesTyped.toLocaleString("pt-BR")} vers.
            </span>
          )}
        </div>
      </div>
